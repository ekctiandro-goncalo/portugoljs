...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --acento:          #e8e8ff;
    --acento-hover:    #ffffff;
    --acento-suave:    rgba(232,232,255,0.08);
    --gradiente-hero:  linear-gradient(160deg, #080810 0%, #0e0e1a 50%, #080810 100%);
    --brilho:          rgba(255,255,255,0.03);
    --sombra-sm: 0 2px 8px rgba(0,0,0,0.5); --sombra-md: 0 8px 32px rgba(0,0,0,0.6);
    --sombra-lg: 0 20px 60px rgba(0,0,0,0.7); --sombra-xl: 0 32px 80px rgba(0,0,0,0.8);
  }
}
...
body {
  font-family: var(--fonte-ui); color: var(--texto); background: var(--fundo);
  line-height: 1.6; overflow-x: hidden; min-height: 100dvh;
}
...
.secao {
  ...
  animation: entrar-baixo var(--dur-slow) var(--ease) both;
}
...
.secao-Nav {
  ...
  animation: entrar-cima var(--dur-slow) var(--ease) both;
}

@media (prefers-color-scheme: dark) {
  .secao-Nav { background: rgba(8,8,16,0.85); }
}
...
.secao-CTA {
  ...
  margin: 0 var(--padding-x) 80px;
  max-width: calc(var(--max) - var(--padding-x) * 2);
}

.secao-CTA .titulo,
.secao-CTA .subtitulo { color: rgba(255,255,255,0.96); }
...
.subtitulo {
  ...
  color: var(--texto-2); margin-bottom: 12px;
}

/* ─── Texto ─── */
.texto {
  font-family: var(--fonte-ui); font-size: 1.05rem;
  line-height: 1.75; color: var(--texto-3); margin-bottom: 8px;
}
...
.botao {
  ...
  font-family: var(--fonte-ui); font-size: 0.9rem; font-weight: 500;
  letter-spacing: 0.01em; color: var(--texto-inv); background: var(--acento);
...
.entrada {
  ...
  padding: 12px 16px; font-family: var(--fonte-ui); font-size: 1rem;
...
.cartao {
  ...
  animation: escalar var(--dur-slow) var(--ease) both;
}

.cartao::before {
  content: ''; position: absolute; inset: 0; border-radius: inherit;
  background: linear-gradient(135deg, var(--brilho) 0%, transparent 50%);
  pointer-events: none;
}
...
.cartao .imagem { margin: -32px -32px 24px; width: calc(100% + 64px); border-radius: var(--raio-lg) var(--raio-lg) 0 0; }
...
.cartao > .icone.lucide,
.cartao .coluna > .icone.lucide {
  width: 2rem; height: 2rem; color: var(--texto);
  padding: 10px; background: var(--fundo-subtil);
...
.ligacao {
  ...
  font-family: var(--fonte-ui); font-size: 0.9rem; font-weight: 450;
...
.distintivo {
  ...
  padding: 5px 14px; font-family: var(--fonte-ui);
  font-size: 0.72rem; font-weight: 500; letter-spacing: 0.07em;
  text-transform: uppercase; color: var(--texto-2);
  background: var(--fundo-subtil); border: 1px solid var(--borda-forte);
  border-radius: var(--raio-full); white-space: nowrap;
  animation: aparecer var(--dur-slow) var(--ease) both;
}
...
.video-contentor {
  ...
  background: var(--fundo-subtil); box-shadow: var(--sombra-md);
}
...
.lista-item {
  ...
  font-family: var(--fonte-ui); font-size: 0.95rem; line-height: 1.6; color: var(--texto-2);
}

.lista-item::before {
  content: ''; display: block; width: 5px; height: 5px; min-width: 5px;
  border-radius: 50%; background: var(--texto-3); margin-top: 8px;
}

/* ─── Citação ─── */
.citacao {
  position: relative; padding: 32px;
  background: var(--fundo-subtil); border: 1px solid var(--borda);
  border-radius: var(--raio-lg); overflow: hidden;
...
.citacao::before {
  ...
  font-family: var(--fonte-display); font-size: 7rem; line-height: 1;
  color: var(--borda-forte); pointer-events: none; user-select: none;
}

.citacao p {
  font-family: var(--fonte-display); font-size: 1.1rem; font-style: italic;
  line-height: 1.75; color: var(--texto); margin-bottom: 16px; position: relative;
}

.citacao-autor {
  display: block; font-family: var(--fonte-ui); font-size: 0.82rem;
  font-style: normal; font-weight: 500; color: var(--texto-3); letter-spacing: 0.02em;
}
